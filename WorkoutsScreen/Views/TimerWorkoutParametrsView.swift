//
//  TimerWorkoutParametrsView.swift
//  MyFirstApp
//
//  Created by Эдгар Исаев on 19.03.2023.
//

import UIKit

protocol NextSetEditingProtocol: AnyObject {
    func nextSetTapped()
    func editingTapped()
}

class TimerWorkoutParametrsView: UIView {
    
    weak var nextSetEditingDelegate: NextSetEditingProtocol?
    ...
    private let timeNumberLabel = UILabel(text: "1 min 30 sec",
                                          font: .robotoMedium24(),
                                          textColor: .specialGray)
    ...
    private func setupViews() {
        ...
        timeNumberLabel.textAlignment = .right
        let stackViewReps = UIStackView(arrangedSubViews: [timeOfSetLabel, timeNumberLabel],
                                        axis: .horizontal,
                                        spacing: 10)
    ...
    @objc private func editingButtonTapped() {
        nextSetEditingDelegate?.editingTapped()
    }
    @objc private func nextSetButtonTapped() {
        nextSetEditingDelegate?.nextSetTapped()
    }
    ...
    public func refreshLabels(model: WorkoutModel, numberOfSet: Int) {
        ...
        timeNumberLabel.text = "\(model.workoutTimer.getTimeFromSeconds())"
    }
    
    public func buttonsIsEnable(value: Bool) {
...
extension TimerWorkoutParametrsView {
